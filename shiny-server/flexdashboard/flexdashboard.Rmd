...
```{r}
  ...
  react_fit = shiny::reactive({
      ...
      sparse_coding = input$sparse_coding
  ...
  react_legend = shiny::reactive({
      kgraph = react_kgraph()
      gglegend = sigmagraph:::get_legend(colors_mapping,
...
```{r}
  ...
  shiny::selectInput('sparse_coding', 'Sparse coding', 
    c('none', 'epsilon'), selected = 'none')
...
```{r}
  ...
  fillPage(sigmagraph::sigmagraphOutput('kg', height = '100%'),
